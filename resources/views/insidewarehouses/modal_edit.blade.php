    <div class="modal fade" id="editModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
       <div class="modal-dialog modal-xl">
          <div class="modal-content">
             <form method="POST" action="{{ route('insidewarehouses.update', $E_PLAN->PLAN_ID) }}">
                @csrf
                @method('PUT')
                   ...
                   <div class="row row-cols-1 row-cols-lg-4">
                      <div class="col mb-3">
                         ...
                         <select class="form-select" id="BUDG_YEAR" name="BUDG_YEAR" required>
                            ...
                            @endphp
                            <option value="{{ $nextYear2 + 543 }}" {{ $E_PLAN->BUDG_YEAR == $nextYear2 + 543 ? 'selected' : '' }}>
                               {{ $displayedNextYear2 }}</option>
                            <option value="{{ $nextYear + 543 }}" {{ $E_PLAN->BUDG_YEAR == $nextYear + 543 ? 'selected' : '' }}>
                               {{ $displayedNextYear }}</option>
                            <option value="{{ $currentYear + 543 }}" {{ $E_PLAN->BUDG_YEAR == $currentYear + 543 ? 'selected' : '' }}>
                      ...
                      <div class="col mb-3">
                         ...
                         <select class="form-select" id="Budget_ID" name="Budget_ID" required>
                            ...
                            @foreach ($E_BUDGET as $BUDGET)
                               <option value="{{ $BUDGET->Budget_ID }}" {{ $E_PLAN->Budget_ID == $BUDGET->Budget_ID ? 'selected' : '' }}>
                                  {{ $BUDGET->Budget_NAME }}
                               </option>
                            @endforeach
                         </select>
                      ...
                      <div class="col mb-3">
                         ...
                         <select class="form-select" id="TYPE_ID" name="TYPE_ID" required>
                            ...
                            @foreach ($E_TYPE as $type)
                               <option value="{{ $type->TYPE_ID }}" {{ $E_PLAN->TYPE_ID == $type->TYPE_ID ? 'selected' : '' }}>
                                  {{ $type->TYPE_NAME }}
                               </option>
                            @endforeach
                         </select>
                      ...
                      <div class="col mb-3">
                         <span>หมวดวัสดุ</span>
                         <input class="form-control" list="categories" id="PRODCT_CAT_ID" name="PRODCT_CAT_ID" placeholder="หมวดวัสดุ" value="{{ $E_PLAN->PRODCT_CAT_ID }}" required>
                         <datalist id="categories">
                            @foreach ($categories as $category)
                               <option value="{{ $category->PRODCT_CAT_ID }}" data-name="{{ $category->PRODCT_CAT_NAME }}"
                                       {{ $E_PLAN->PRODCT_CAT_ID == $category->PRODCT_CAT_ID ? 'selected' : '' }}>
                                  {{ $category->PRODCT_CAT_NAME }}
                               </option>
                            @endforeach
                         </datalist>
                         <input type="hidden" id="PRODCT_CAT_NAME" name="PRODCT_CAT_NAME" value="{{ $E_PLAN->PRODCT_CAT_NAME }}">
                      </div>
                   </div>
                </div>
