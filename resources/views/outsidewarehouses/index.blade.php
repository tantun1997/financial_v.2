    ...
    @section('contents')
                   ...
                   <tr>
                      ...
                      <td class="text-center align-middle">
                         <div class="btn-group mb-2" role="group">
                            ...
                            <form action="{{ route('outsidewarehouses.destroy', $item->PLAN_ID) }}" method="POST" onsubmit="return confirm('ต้องการปิดใช้งานหรือไม่?')" class="p-0">
                               @csrf
                               @method('DELETE')
                         ...
                         <div>
                            ...
                            @if (Auth::user()->id == '114000041')
                               @if ($item->REQ_APPROVED_ID == 1)
                                  <form id="approval-form-{{ $item->PLAN_ID }}" action="{{ route('outsidewarehouses.approved', $item->PLAN_ID) }}" method="POST">
                                     @csrf
                                     @method('PATCH')
       ...
       <script>
          function exportFilteredData() {
             ...
             const exportUrl = `{{ route('export.data', ['type' => 'outsidewarehouses']) }}?PLAN_SET_YEAR=${PLAN_SET_YEAR}&TCHN_LOCAT_NAME=${TCHN_LOCAT_NAME}`;
          ...
          function exportFilteredData_items() {
             ...
             // สร้างลิงก์ส่งออกด้วยข้อมูลตัวกรอง
             const exportUrl = `{{ route('export.data', ['type' => 'outsidewarehouses_items']) }}?PLAN_SET_YEAR=${PLAN_SET_YEAR}&TCHN_LOCAT_NAME=${TCHN_LOCAT_NAME}`;
