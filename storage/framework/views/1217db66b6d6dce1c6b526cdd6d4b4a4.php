    ...
    <?php $__env->startSection('title', 'แผนวัสดุในคลัง'); ?>
    ...
    <?php $__env->startSection('contents'); ?>
       ...
       <div class="mb-3">
          <?php if($E_CLOSE_PLAN->status == 'Y'): ?>
             <button type="button" class="btn btn-primary" id="addItemButton" data-bs-toggle="modal" data-bs-target="#addModal_product" data-action="<?php echo e(route('insidewarehouses.store')); ?>">
                + เพิ่มรายการ
             </button>
          <?php endif; ?>
                   ...
                   <tr>
                      ...
                      <td class="text-center align-middle">
                         <div class="btn-group mb-2" role="group">
                            <a href="<?php echo e(route('insidewarehouses.show', $item->PLAN_ID)); ?>" type="button" class="btn btn-outline-primary btn-sm" style="white-space: nowrap;">ดูข้อมูล</a>
                            <form action="<?php echo e(route('insidewarehouses.destroy', $item->PLAN_ID)); ?>" method="POST" onsubmit="return confirm('ต้องการปิดใช้งานหรือไม่?')" class="p-0">
                               <?php echo csrf_field(); ?>
                         ...
                         <div>
                            <?php if(Auth::user()->id != '114000041'): ?>
                               <form id="approval-form" action="<?php echo e(route('insidewarehouses.approved', $item->PLAN_ID)); ?>" method="POST">
                                  <?php echo csrf_field(); ?>
                            ...
                            <?php if(Auth::user()->id == '114000041'): ?>
                               <?php if($item->REQ_APPROVED_ID == 1): ?>
                                  <form id="approval-form-<?php echo e($item->PLAN_ID); ?>" action="<?php echo e(route('insidewarehouses.approved', $item->PLAN_ID)); ?>" method="POST">
                                     <?php echo csrf_field(); ?>
                                     <?php echo method_field('PATCH'); ?>
       ...
       <!-- Include the modal -->
       <?php echo $__env->make('insidewarehouses.modal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
       ...
       <script>
          function exportFilteredData() {
             ...
             const exportUrl = `<?php echo e(route('export.data', ['type' => 'insidewarehouses'])); ?>?PLAN_SET_YEAR=${PLAN_SET_YEAR}&TCHN_LOCAT_NAME=${TCHN_LOCAT_NAME}`;
          ...
          function exportFilteredData_items() {
             ...
             // สร้างลิงก์ส่งออกด้วยข้อมูลตัวกรอง
             const exportUrl = `<?php echo e(route('export.data', ['type' => 'insidewarehouses_items'])); ?>?PLAN_SET_YEAR=${PLAN_SET_YEAR}&TCHN_LOCAT_NAME=${TCHN_LOCAT_NAME}`;
          ...
          document.addEventListener('DOMContentLoaded', function() {
             ...
             document.querySelectorAll('.form-check-input').forEach((checkbox) => {
                checkbox.addEventListener('change', function() {
                   ...
                   fetch('<?php echo e(route('insidewarehouses.close_plan')); ?>', {
                         method: 'POST',
                         headers: {
                         ...
                         if (status === 'Y') {
                            if (!addItemButton) {
                               ...
                               button.setAttribute('data-action',
                                  '<?php echo e(route('insidewarehouses.store')); ?>');
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/financial_test/resources/views/insidewarehouses/index.blade.php ENDPATH**/ ?>